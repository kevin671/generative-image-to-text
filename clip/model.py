    ...
    def attention(self, x: torch.Tensor, attn_mask=None):
        if attn_mask is None:
            ...
            if self.attn_mask is not None:
                num = x.shape[0]
                attn_mask = self.attn_mask[:num, :num]
            else:
                attn_mask = None
...
class VisualTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int,
        ...
        self.output_grid = output_grid
        self.grid_after_ln = grid_after_ln

    def forward(self, x: torch.Tensor):
        x = x.to(self.conv1.weight.dtype)
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        expected_dim = self.input_resolution // self.conv1.kernel_size[0]
        pos_embed = self.positional_embedding.to(x.dtype)
        if x.shape[2] != expected_dim or x.shape[3] != expected_dim:
            embed_dim = len(self.class_embedding)
            pos_embed = pos_embed[1:, :].reshape((expected_dim, expected_dim, embed_dim))
            new_size = (x.shape[2], x.shape[3])
            ...
            pos_embed = torch.cat((self.positional_embedding[0:1, :], pos_embed), dim=0)
...
def build_model(state_dict: dict, skip_load=False):
